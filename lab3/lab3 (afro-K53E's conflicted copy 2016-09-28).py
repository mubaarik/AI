...
def get_minimax_score(state, player, path, paths, depth_limit, heuristic_fn):
    ...
    if depth==depth_limit:
        paths.append(_path)
        board=state.get_snapshot()
        #print "h_fn: ", heuristic_fn(board, player)
        return _path, heuristic_fn(board, player)
        depth+=1
...
def generic_minmax_search(state, alpha=-INF, beta=INF, heuristic_fn=always_zero, depth_limit=INF, maximize=True):
    ...
    if maximize:
        ...
        for child in state.generate_next_states():
            [_state, score, evals] = minimax_search(child, heuristic_fn, depth_limit - 1, False)
            num_avals+=evals
            if score > _score:
                _score = score
                _path = path[:]+_state
            ...
            if alpha >= beta:
                return [_path, alpha, num_avals]
        
        return [_path, _score, num_avals]
    else:
        _score = INF
        for child in state.generate_next_states():
            [_state, score, evals] = minimax_search(child, heuristic_fn, depth_limit - 1, True)
            num_avals+=evals
            if score < _score:
                _score = score
                _path = path[:]+_state
...
def minimax_search(state, heuristic_fn=always_zero, depth_limit=INF, maximize=True) :
    ...
    #return minimax_endgame_search(state, maximize)
